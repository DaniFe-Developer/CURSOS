...
def pedirDatosRegistro():
    ...
    while not creditoCorrecto:
        Creditos = input("Ingresa los créditos: ")
        if Creditos.isdigit() and int(Creditos) > 0:
            creditoCorrecto = True
            Creditos = int(Creditos)
        else:
            print("Créditos incorrectos: Debe ser numérico y mayor a 0")
...
def pedirDatosEliminacion(cursos):
    ...
    if not existeCodigo:
        ...
        codigoEliminar = ""  # Setting it to an empty string if the code doesn't exist
